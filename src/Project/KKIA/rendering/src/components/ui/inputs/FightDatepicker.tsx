...
const FlightDatepicker: React.FC<FlightDatepickerProps> = ({
  ...
  // Set initial value to today's date if value is not provided
  useEffect(() => {
    if (!value) {
  ...
  return (
    <div className="relative h-10 w-full max-w-[150px] cursor-pointer" ref={calendarRef}>
      <input
        id={id}
        className={`h-10 bg-background-primary text-body-normal-regular text-text-secondary p-2 pr-10 w-full max-w-[150px] outline-none cursor-pointer`}
        type="text"
        placeholder={placeholder || 'Select a date'}
        value={formatValue(value || format(new Date(), 'yyyy-MM-dd'))}
        onFocus={() => setShowCalendar(true)}
        readOnly
        required={required}
      />
      <div className="absolute right-2 top-2 h-6 w-6">
        {showCalendar ? <ChevronUpSvg /> : <ChevronDownSvg />}
      </div>
